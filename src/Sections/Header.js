import styles from "./Header.module.css";
import icon from "../Icon/house.png";
import { signOut } from "firebase/auth";
import { signInWithGoogle, auth } from "../firebase-config.js";
import React, { useState } from "react";
function Header(props) {
  const [cred, oncred] = useState("");
  props.credval(cred);
  return (
    <React.Fragment>
      <div className={styles.header}>
        <div className={styles.icon}>
          <img src={icon} />
          <h1 className={styles.title}>Estate</h1>
        </div>
        <div className={styles.rent}>Rent</div>
        <div className={styles.buy}>Buy</div>
        <div className={styles.sell}>Sell</div>
        <div className={styles.manage}>ManageProperty</div>
        {!sessionStorage.getItem("user") && (
          <button
            className={styles.header_btn}
            onClick={(e) => {
              signInWithGoogle()
                .then((result) => {
                  oncred(result);
                  sessionStorage.setItem("user", JSON.stringify(result));
                })
                .catch((error) => {
                  console.log(error);
                });
            }}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              x="0px"
              y="0px"
              width="48"
              height="48"
              viewBox="0 0 48 48"
              className={styles.svgs}
              // style=" fill:#undefined;"
            >
              <path
                fill="#FFC107"
                d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"
              ></path>
              <path
                fill="#FF3D00"
                d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"
              ></path>
              <path
                fill="#4CAF50"
                d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"
              ></path>
              <path
                fill="#1976D2"
                d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"
              ></path>
            </svg>
            <p>Signin With Google</p>
          </button>
        )}
        {sessionStorage.getItem("user") && (
          <React.Fragment>
            <div
              className={styles.logout}
              onClick={(e) => {
                signOut(auth)
                  .then(() => {
                    oncred("");
                    sessionStorage.clear();
                    window.location.reload();
                  })
                  .catch((error) => {
                    console.log(error);
                  });
              }}
            >
              <svg
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                width="20"
                height="20"
                viewBox="0 0 20 20"
                className={styles.logoutsvg}
              >
                <title>log-out</title>
                <path d="M19 10l-6-5v3h-7v4h7v3l6-5zM3 3h8v-2h-8c-1.1 0-2 0.9-2 2v14c0 1.1 0.9 2 2 2h8v-2h-8v-14z"></path>
              </svg>
              <span className={styles.logout_text}>Logout</span>
            </div>
          </React.Fragment>
        )}
      </div>
      {!sessionStorage.getItem("user") && (
        <div className={styles.loggedout}>Login To View....</div>
      )}
    </React.Fragment>
  );
}
export default Header;
